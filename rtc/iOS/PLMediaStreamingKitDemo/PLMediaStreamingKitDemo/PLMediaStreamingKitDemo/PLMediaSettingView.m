...
@interface PLMediaSettingView () <UITextFieldDelegate>
{
    ...
    PLMediaSettingManager *_currentSettings;
}
...
@implementation PLMediaSettingView
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        [self setupSubviews];
        self.settings = [[PLMediaSettingManager alloc] init];
        
        _currentSettings = [[PLMediaSettingManager alloc] init];
    }
    return self;
}
...
- (void)addMediaSettingControlView {
    ...
    // 视频平均码率
    UILabel *averageVideoBitRateLabel = [self getLabelView:@"视频平均码率 Kbps：" andFrame:CGRectMake(20, 320, 140, 30)];
    ...
    _averageVideoBitRateSegmentedControl = [self getUISegmentedControlView:_averageVideoBitRateArray andFrame:CGRectMake(10, 350, CGRectGetWidth([UIScreen mainScreen].bounds) - 20, 30)];
    [self addSubview:_averageVideoBitRateSegmentedControl];
    _averageVideoBitRateSegmentedControl.selectedSegmentIndex = 1;
    
    // 视频编码器类型
    UILabel *videoEncoderLabel =[self getLabelView:@"视频编码器类型：" andFrame:CGRectMake(20, 390, 120, 30)];
    ...
    _videoEncoderSegmentedControl = [self getUISegmentedControlView:_videoFrameRateArray andFrame:CGRectMake(videoEncoderLabel.frame.origin.x + 120, 390, CGRectGetWidth([UIScreen mainScreen].bounds) - videoEncoderLabel.frame.origin.x - 130, 30)];
    [self addSubview:_videoEncoderSegmentedControl];
    _videoEncoderSegmentedControl.selectedSegmentIndex = 0;
    
    // 保存按钮
    _saveButton = [UIButton buttonWithType:UIButtonTypeCustom];
    _saveButton.frame = CGRectMake(CGRectGetWidth([UIScreen mainScreen].bounds) / 4, 435, CGRectGetWidth([UIScreen mainScreen].bounds) / 2, 45);
...
- (void)saveButtonClick:(id)sender {
    ...
    NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
    [userDefaults setObject:self.roomNametextField.text forKey:@"PLMediaRoomName"];
//    [userDefaults setInteger:self.audioOnlySegmentControl.selectedSegmentIndex forKey:@"PLMediaAudioOnly"];
    ...
    if (![_currentSettings isEqualObj:self.settings]) {
        ...
        if ([self.delegte respondsToSelector:@selector(mediaSettingView:)]) {
            [self.delegte mediaSettingView:self];
        }
    }
    else {
